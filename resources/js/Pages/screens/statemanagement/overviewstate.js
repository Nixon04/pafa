...
export const useListStore = defineStore('all_lists', () => {
    ...
    const secondquestion = ref([
        {
            'id': 1,
            'name': 'I want my child to learn faster and do better in school', 
        },
        {
            'id': 2,
            'name': ' have a child with special needs and want to support their learning', 
        },
        {
            'id': 3,
            'name': 'I want to sharpen my mind and think faster ', 
        },
        {
            'id': 4,
            'name': 'I want to improve creativity and problem-solving', 
        }
    ]);
    ...
    const fifthquestion = ref([
        {
           ...
           {
            'id': 2,
            'name':   'A little, but not seriously',
           },
           {
            'id': 3,
            'name':   'No, but I’m open to learning',
           },
           {
            'id': 4,
            'name':   'No, and I don’t think I’m good at it',
           }
    ]);
    

    const sixquestion = ref([
        {
            'id': 1,
            'name':   'Yes, definitely! ',
           },
           {
            'id': 2,
            'name':   'Yes, but I may need reminders',
           },
           {
            'id': 3,
            'name':   'Not sure ',
    ...
    function changeindex(value, name){
      answers.value[currentstep.value] = name;
      console.log('inserted ',  answers.value );
    }
    ...
    function updateAnswer() {
        answers.value[6] = number.value; // Automatically update the answer at step 6
        localStorage.setItem('answers', JSON.stringify(answers.value)); // Save to local storage
        console.log('Updated answers: ', number.value); // Debugging
    }
    ...
    const submit = async ()  => {
        ...
        if (!userdetails.fullname || !userdetails.contact || !userdetails.email) {
            toast.error('required fields empty');
            isLoading.value = false;
            return;
        }

        if(answers.value[0] == "My Child" ||  answers.value[0] == 'A family member' && !userdetails.members){
            toast.error('Member count needed');
            isLoading.value = false;
            return;
        }
        ...
        if (!emailRegex.test(userdetails.email)) {
            ...
            return;
                ...
                const fullPhoneNumber = userdetails.countryCode + userdetails.contact;
